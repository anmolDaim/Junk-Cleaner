...
import com.example.junkcleaner.Adapter.AppManagerAdapter
import com.example.junkcleaner.Adapter.itemAppManagerAdapter
import com.example.junkcleaner.DataClass.appManagerDataClass
import com.example.junkcleaner.DataClass.itemAppManagerDataClass
import com.example.junkcleaner.R
import com.example.junkcleaner.databinding.ActivityAppManagerBinding
import java.io.File
import java.util.Locale

class AppManagerActivity : AppCompatActivity(), itemAppManagerAdapter.OnCheckboxChangeListener,itemAppManagerAdapter.DetailsClickListner {
...
//                if (isSystemApp) {
          ...
          //  }
            ...
            binding.appManagerItemRecyclerView.layoutManager = LinearLayoutManager(this)
            binding.appManagerItemRecyclerView.adapter = itemAppManagerAdapter(appList, this,this)
    ...
    private fun fetchSize() {
        ...
        try {
            ...
            val sortedAppList = appList.sortedByDescending { it.appSize?.replace(" MB", "")?.toIntOrNull() }

            binding.appManagerItemRecyclerView.layoutManager = LinearLayoutManager(this)
            binding.appManagerItemRecyclerView.adapter = itemAppManagerAdapter(sortedAppList, this,this)
    ...
    private fun backupApp(packageName: String?, appName: String?) {
        ...
        try {
            ...
            // Copy the APK file to the backup directory
            srcFile.copyTo(destFile, overwrite = true)

            Toast.makeText(this, "Backup successful: ${destFile.absolutePath}", Toast.LENGTH_LONG).show()
        } catch (e: Exception) {
            e.printStackTrace()
